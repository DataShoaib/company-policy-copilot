...
def _consume_refresh_jti(payload: dict) -> bool:
    """Mark the refresh-token JTI consumed; returns False on replay."""
    jti = payload.get("jti")
    if not jti:
        return True
    return consume_refresh_jti(
        jti=jti,
        exp=payload.get("exp"),
        ttl_fallback_seconds=settings.refresh_token_expire_days * 86400,
    )
...
def refresh(body: RefreshRequest):
    ...
    if not _consume_refresh_jti(payload):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Refresh token already used — request a new token pair.",
        )
